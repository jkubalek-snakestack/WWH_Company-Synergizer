...
class Capability:
    ...
    def from_dict(payload: Dict) -> "Capability":
        channels = []
        for channel in payload.get("engagement_channels", []):
            try:
                channels.append(EngagementChannel(channel))
            except ValueError:
                raise ValueError(f"Invalid engagement channel: {channel}") from None
        return Capability(
            name=payload["name"],
...
class CompanyProfile:
    ...
    def from_dict(payload: Dict) -> "CompanyProfile":
        ...
        if "slug" not in payload or not payload.get("slug"):
            raise ValueError("CompanyProfile missing required field: slug")
        if "name" not in payload or not payload.get("name"):
            raise ValueError("CompanyProfile missing required field: name")
        
        # Validate list fields before iterating
        offerings_data = payload.get("offerings", [])
        if not isinstance(offerings_data, list):
            raise ValueError(f"CompanyProfile field 'offerings' must be a list, got {type(offerings_data).__name__}")
        
        needs_data = payload.get("needs", [])
        if not isinstance(needs_data, list):
            raise ValueError(f"CompanyProfile field 'needs' must be a list, got {type(needs_data).__name__}")
        
        assets_data = payload.get("assets", [])
        if not isinstance(assets_data, list):
            raise ValueError(f"CompanyProfile field 'assets' must be a list, got {type(assets_data).__name__}")
        
        initiatives_data = payload.get("initiatives", [])
        if not isinstance(initiatives_data, list):
            raise ValueError(f"CompanyProfile field 'initiatives' must be a list, got {type(initiatives_data).__name__}")
        
        key_contacts_data = payload.get("key_contacts", [])
        if not isinstance(key_contacts_data, list):
            raise ValueError(f"CompanyProfile field 'key_contacts' must be a list, got {type(key_contacts_data).__name__}")
        ...
        return CompanyProfile(
            ...
            offerings=[Capability.from_dict(item) for item in offerings_data],
            needs=[Need.from_dict(item) for item in needs_data],
            assets=[Asset.from_dict(item) for item in assets_data],
            initiatives=[Initiative.from_dict(item) for item in initiatives_data],
            key_contacts=[Contact.from_dict(item) for item in key_contacts_data],
